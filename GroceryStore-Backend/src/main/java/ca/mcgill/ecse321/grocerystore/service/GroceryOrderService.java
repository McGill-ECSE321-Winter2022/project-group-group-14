...
public class GroceryOrderService {
	...
	public GroceryOrder createDeliveryOrder(Customer customer) {
		...
		// checks if customer has a current active order, they should only have a
		// completed order
	...
	public GroceryOrder createInStoreOrder() {
		...
		int totalCost = 0;
		order.setTotalCost(totalCost);
		order.setOrderStatus(OrderStatus.Received); // when employee is done adding items, the order will be marked as
													// completed
		order = orderDao.save(order);
		return order;
	}
	...
	@Transactional
	public GroceryOrder placeOrder(GroceryOrder order) { // SHOULD ONLY BE CALLED ONCE WE ARE DONE ADDING ITEMS TO THE
															// COMPLETED ORDER
	...
	public GroceryOrder addOrderItems(GroceryOrder order, List<OrderItem> orderItems) {
		...
		if (!inventoryItemDao.findByName(orderItems.get(0).getName()).getAvailability())
			throw new IllegalArgumentException("Item is only available in store.");
...
//    }
	...
	public GroceryOrder deleteItemFromOrder(GroceryOrder groceryOrder, String itemName) {
		...
		if (!groceryOrder.getOrderStatus().equals(OrderStatus.Received))
			throw new IllegalArgumentException("Can only remove items from an order that has not been placed yet!");
...
//    
	...
	public void deleteAllCompletedOrders() {
		...
		for (GroceryOrder order : orderDao.findByOrderStatus(OrderStatus.Completed)) { // only delete complete grocery
																						// orders
	...
	@Transactional
	public GroceryOrder toggleOrderType(GroceryOrder order) { // should only be accessed by owner --> this will be dealt
																// with in the view
		OrderType type = order.getOrderType();
		if (type.equals(OrderType.Delivery)) {
			if (!order.getCustomer().getAddress().contains(GroceryStore.town)) { // if out of town delivery order remove
																					// 10$ when switching
			...
			if (!order.getCustomer().getAddress().contains(GroceryStore.town)) { // if out of town delivery order add
																					// 10$ when switching to delivery
				order.setTotalCost(order.getTotalCost() + GroceryStore.outOfTownFee);
			}
			order.setOrderType(OrderType.Delivery);
		}
	 ...
	 */
	@Transactional
	public GroceryOrder updateOrderStatus(GroceryOrder order) { // should only be done by an employee/owner account -->
																// we will take care of this in the view for the ui
		if (order == null)
		...
		if (order.getOrderStatus().equals(OrderStatus.Completed)) { // if order is already completed, order status no
																	// longer needs to be updated
			throw new IllegalArgumentException("Order has already been completed. Does not require an update.");
		}
		if (order.getOrderStatus() == null) { // if order is already completed, order status no longer needs to be
												// updated
			throw new IllegalArgumentException("Order has no status.");
		}
		if (order.getOrderStatus().equals(OrderStatus.Received)) { // should have already been updated once order had
	 ...
	 */
	@Transactional
	public OrderStatus viewOrderStatus(GroceryOrder order) { // customer should be able to view the status of their
																// orders
		if (order == null)
